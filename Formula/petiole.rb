...
class Petiole < Formula
  ...
  def install
    ...
    (bin/"petiole").write <<~SHELL
      #!/bin/bash
      echo "petiole: Reserved under the Canopy ecosystem." >&2
      echo "This tool is not yet released publicly." >&2
      exit 1
    SHELL
    chmod 0755, bin/"petiole"
  end

  test do
    assert_match "reserved under the Canopy ecosystem", shell_output("#{bin}/petiole 2>&1", 1)
  end

  def caveats
    <<~EOS
      petiole: Reserved under the Canopy ecosystem.
