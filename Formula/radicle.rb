...
class Radicle < Formula
  ...
  def install
    ...
    (bin/"radicle").write <<~SHELL
      #!/bin/bash
      echo "radicle: Reserved under the Canopy ecosystem." >&2
      echo "This tool is not yet released publicly." >&2
      exit 1
    SHELL
    chmod 0755, bin/"radicle"
  end

  test do
    assert_match "reserved under the Canopy ecosystem", shell_output("#{bin}/radicle 2>&1", 1)
  end

  def caveats
    <<~EOS
      radicle: Reserved under the Canopy ecosystem.
